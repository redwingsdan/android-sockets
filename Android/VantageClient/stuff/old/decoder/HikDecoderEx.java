...
import com.android.SimplePlayer.BMPImage;
...
import com.org.MediaPlayer.PlayM4.*;
import com.org.MediaPlayer.PlayM4.PlayerCallBack.PlayerDisplayCB;
...
public class HikDecoderEx implements IVideoDecoder {
	...
	public boolean start(CodecID videoTpe, InputStream inputStream,
		...
		boolean ret = false;
		ret = _player.setStreamOpenMode(_port, Constants.STREAME_REALTIME);
		ret = _player.openStream(_port, head, 40, 1024 * 1024);
		ret = _player.setDisplayBuf(_port, 1024 * 1024);
		// ret = _player.setDisplayCB(_port, new DisplayCS());
		ret = _player.play(_port, _holder);
	...
	//UNUSED
	/*
	class DisplayCS implements PlayerDisplayCB {
		...
		public void onDisplay(int arg0, ByteBuffer arg1, int arg2, int arg3,
			...
			if (_callBack != null) {
				...
				byte RGBData[] = new byte[arg3 * arg4];
				byte[] mYUVData = new byte[arg2];
				arg1.get(mYUVData, 0, arg2);

				decodeYUV420SP(RGBData, mYUVData, arg3, arg4);

				byte[] bitmapByteArray = new BMPImage(RGBData, arg3, arg4)
						.getByte();
				Bitmap bitmap = BitmapFactory.decodeByteArray(bitmapByteArray,
						0, bitmapByteArray.length);

				_callBack.mjpegDataReceived(bitmap);
			}
		}
	}
*/
